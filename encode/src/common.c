...
int read_bit_alloc(int table, al_table* alloc)        /* read in table, return # subbands */
{
        ...
        fclose(fp);
#endif
extern unsigned int alloc_0_data[];
extern unsigned int alloc_1_data[];
extern unsigned int alloc_2_data[];
extern unsigned int alloc_3_data[];
extern unsigned int alloc_4_data[];

        unsigned int sblim, i, j, *ap;
	        
	switch (table) {
	 default:	/* fall through */
	 case 0:	ap = alloc_0_data;	break;
	 case 1:	ap = alloc_1_data;	break;
	 case 2:	ap = alloc_2_data;	break;
	 case 3:	ap = alloc_3_data;	break;
	 case 4:	ap = alloc_4_data;	break;
	}
...
enum byte_order DetermineByteOrder()
{
	...
	{
		short val;
		char  charval[ sizeof(short) ];
	} probe;
	
	probe.val = 0x4142L;  /* AB in ASCII */
	if (probe.charval[0] == 'A' &&
	    probe.charval[1] == 'B') {
		printf ("Big endian byte order\n");
		return order_bigEndian;
	} else {
		if (probe.charval[0] == 'B' &&
		    probe.charval[1] == 'A') {
